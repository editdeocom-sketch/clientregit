...
export default function VideoDetailPage() {
      ...
      try {
        ...
        if (res.data) {
          setComments(res.data.map(c => ({
            id: String(c.id),
            user: String(c.user_name || c.userName || c.user || "User"),
  ...
  const duration = mediaDuration || videoData?.duration || 155
  ...
  const togglePlayback = () => {
    ...
    if (videoRef.current.paused) videoRef.current.play().catch(() => setMediaError(true))
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await api.post<{ data: Record<string, unknown> }>(`/videos/${id}/comments`, {
      ...
      setComments((prev) => [...prev, {
        id: String(saved.id),
        user: String(saved.user_name || user?.name || "You"),
  ...
  const handleApprove = async () => {
    try {
      await api.put(`/videos/${id}/status`, { status: "approved" })
      setStatus("approved")
      setApprovalOpen(false)
      toast.success("Video approved!")
    } catch {
      toast.error("Unable to approve video")
    }
  }

  const handleRevision = async () => {
    try {
      await api.put(`/videos/${id}/status`, { status: "revision_requested" })
          ...
          >
            {videoData?.file_url && !mediaError ? (
              ...
              <div className="flex flex-col items-center gap-4 text-center px-6">
                ...
                <div>
                  <p className="font-medium text-foreground">Media unavailable</p>
                  <p className="mt-1 text-sm text-muted-foreground">This video has no playable local file.</p>
                </div>
              </div>
            )}
